...
echo "Running orat..."
cp ablation/main_mrat.rs src/main.rs
for (( i=0; i<$NUM_RUNS; i++ ))
do
  (time cargo run --release --bin ruler -- --domain $DOMAIN --variables 3 --iters $NUM_ITERS --rules-to-take 1) &> "$OUTPUT_DIR/orat/${DOMAIN}_3-${NUM_ITERS}_$i.log" 
done
...
do
    for (( i=0; i<$NUM_RUNS; i++ ))
    do
        (time cargo run --release --bin ruler -- --domain $DOMAIN --variables 3 --iters $NUM_ITERS --rules-to-take $r) &> "$OUTPUT_DIR/mrat/${DOMAIN}_3-${NUM_ITERS}_$r-$i.log" 
    done
done

echo "Running minimize and phase-times..."
cp ablation/main_minimize.rs src/main.rs
cp ablation/lib.rs src/lib.rs
for (( i=0; i<$NUM_RUNS; i++ ))
do
  (time cargo run --release --bin ruler -- --domain $DOMAIN --variables 3 --iters $NUM_ITERS --rules-to-take 1) &> "$OUTPUT_DIR/minimize/${DOMAIN}_3-${NUM_ITERS}_$i.log" 
  # it's the same but save it in a different location
  (time cargo run --release --bin ruler -- --domain $DOMAIN --variables 3 --iters $NUM_ITERS --rules-to-take 1) &> "$OUTPUT_DIR/phase-times/${DOMAIN}_3-${NUM_ITERS}_$i.log" 
done
...
for (( i=0; i<$NUM_RUNS; i++ ))
do
  (time cargo run --release --bin ruler -- --domain $DOMAIN --variables 3 --iters $NUM_ITERS --rules-to-take 1) &> "$OUTPUT_DIR/no-run-rewrites/${DOMAIN}_3-${NUM_ITERS}_$i.log" 
done
