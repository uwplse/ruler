...
impl SynthLanguage for Math {
    ...
    fn init_synth(synth: &mut Synthesizer<Self>) {
        ...
        let constants: Vec<u4> = vec![0.into(), 1.into(), 2.into()];
...
pub fn egg_to_z3<'a>(ctx: &'a z3::Context, expr: &[Math]) -> z3::ast::BV<'a> {
    ...
    for node in expr.as_ref().iter() {
        match node {
            Math::Var(v) => buf.push(ast::BV::new_const(&ctx, v.to_string(), 4)),
            Math::Num(c) => buf.push(ast::BV::from_u64(&ctx, c.0 as u64, 4)),
...
impl_bits!(std::primitive::u64, u4, 4);
