...
pub unsafe extern "C" fn egraph_run_with_iter_limit(
    ...
    ffirun(|| {
        ...
        if runner.stop_reason.is_none() {
            ...
            // The scheduler
            let limits = Limits::new(iter_limit as usize, node_limit as usize);
            let scheduler = Scheduler::Saturating(limits);

            // The rules
            let rules = rules::mk_rules(&ffi_tuples);
            let ruleset = ruleset::mk_ruleset(&ffi_tuples);

            // Run
            runner = scheduler.run(runner, &ruleset);
            ctx.rules = rules;
        }

        let iter_len = runner.iterations.len();
        std::ptr::write(output_size, iter_len as u32);
