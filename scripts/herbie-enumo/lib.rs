...
use egg::{Extractor, Id, Iteration, Language, StopReason, Symbol, BackoffScheduler};
...
pub unsafe extern "C" fn egraph_run_with_iter_limit(
    ...
    ffirun(|| {
        ...
        if runner.stop_reason.is_none() {
            ...
            let mut scheduler = BackoffScheduler::default().with_ban_length(1);
            for rule in &rules {
                scheduler = scheduler.rule_match_limit(rule.name, 100_000);
            }
