...
# This is hacky. Basically all appearances
# of the simplify tag in rulesets are either
# tag1 tag2 ... simplify fp-safe) or tag1 simplify fp-safe-nan) ...
def remove_simplify_fp_tag(f):
    f.seek(0)
    lines = f.readlines()
    ret = []
    for l in lines:
        # remove fp-safe and simplify tags from all rules except bools and branch ones
        # ruler does not currently generate bools and conditionals together with fp safe
        if (("simplify fp-safe)" in l) and (not ("bool-reduce" in l)) and (not ("branch-reduce" in l)) and (not ("compare-reduce" in l))):
            ret.append(l.replace(" simplify fp-safe)", ")"))
        elif (("simplify fp-safe-nan)" in l) and (not ("bool-reduce" in l)) and (not ("branch-reduce" in l)) and (not ("compare-reduce" in l))):
            ret.append(l.replace(" simplify fp-safe-nan)", ")"))
        else:
            ret.append(l)
    return ret
...
def mk_rkt_rules(rules, rkt, unique_vars, config):
    if config == "herbie-only":
        with open(rkt, 'w') as f:
            f.write(prelude)
    elif config == "herbie-no-fp-simpl":
        with open(rkt, 'w+') as f:
            f.write(prelude)
            lines = remove_simplify_fp_tag(f)
        with open(rkt, 'w') as f:
            for l in lines:
                f.write(l)
        ...
        with open(rkt, 'a') as f:
            # we are only using ruler's rules for simplify, at least for fp ruler
            f.write("\n\n(define-ruleset* ruler-rules (ruler simplify fp-safe)\n")
            f.write("#:type (")
        ...
        Select from: herbie-only | herbie-no-fp-simpl | herbie-ruler | ruler-only""")
...
usage = """USAGE: python preprocess.py <input> <output> <config>
    ...
    <config> herbie-only | herbie-no-fp-simpl | herbie-ruler | ruler-only"""
